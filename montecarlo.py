...
def tabla_montecarlo_A(tabla_demanda, tabla_demora, tabla_pedido, dias, dia_llegada, cant):
    ...
    for i in range (dias):
        ...
        resolucion = Resolucion(i,rnd_demanda,demanda,dia_llegada,rnd_demora,demora,stock_anterior,decenas_pedidas,costo_orden,costo_mantenimiento,costo_ruptura,costo_total,costo_acumulado, promedio,0)
...
def tabla_montecarlo_b(tabla_demanda, tabla_demora, tabla_pedido, dias, dia_llegada):
    stock_anterior = 200
    resoluciones = []
    pendiente_entrada = False
    ...
    for i in range(dias):
        ...
        if dia_llegada == i:
            if i != 0:
                decenas_pedidas = resoluciones[i-1].acumulador_demanda
                acumulador_demanda = 0
            dia_llegada += intervalo_pedido  # Defino el proximo día que llegan los pedidos
            rnd_demora = random.uniform(0, 1)
            pendiente_entrada = True
        ...
        if pendiente_entrada and demora == i:
            stock_anterior +=  decenas_pedidas
            decenas_pedidas = 0
            pendiente_entrada = False
            demora = 0
        ...
        if i == 0:
            costo_acumulado = costo_total
            decenas_pedidas = demanda
        else:
            costo_acumulado = costo_total + resoluciones[i - 1].acumulador_costo

        promedio = costo_acumulado / (i + 1)

        resolucion = Resolucion(i, rnd_demanda, demanda, dia_llegada, rnd_demora, demora, stock_anterior,
                                decenas_pedidas, costo_orden, costo_mantenimiento, costo_ruptura, costo_total,
                                costo_acumulado, promedio, acumulador_demanda)
